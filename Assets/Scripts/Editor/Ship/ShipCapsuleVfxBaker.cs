...
///   FireEmbers) into the existing CapsuleTrailVfx prefab.
/// - <see cref="CreateCapsuleImpactPrefab"/> creates a brand new "CapsuleImpactVfx" prefab (Impact
...
{
    const string TrailPrefabPath = "Assets/Resources/Ship/Capsule/CapsuleTrailVfx.prefab";
    const string ImpactPrefabPath = "Assets/Resources/Ship/Capsule/CapsuleImpactVfx.prefab";

    [MenuItem("Tools/Ship VFX/Bake Fire Trail Into CapsuleTrailVfx")]
    public static void Bake()
    {
        GameObject root = PrefabUtility.LoadPrefabContents(TrailPrefabPath);
        try
        {
            EnsureChildWithComponent<ShipFireTrail>(root.transform, "FireTrail");

            PrefabUtility.SaveAsPrefabAsset(root, TrailPrefabPath);
            Debug.Log($"ShipCapsuleVfxBaker: baked FireTrail into {TrailPrefabPath}");
        }
        finally
        {
    ...
    /// <summary>Builds a fresh "CapsuleImpactVfx" prefab asset (ImpactFlash/ImpactDust/ImpactSparks/
    ...
    [MenuItem("Tools/Ship VFX/Create CapsuleImpactVfx Prefab")]
    public static void CreateCapsuleImpactPrefab()
    {
        ...
        GameObject root = prefabExists
            ...
            : new GameObject("CapsuleImpactVfx");

        GameObject builder = new GameObject("CapsuleImpactVfx_Builder");
        try
        {
